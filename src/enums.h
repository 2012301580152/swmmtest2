...
//-------------------------------------
 enum ObjectType {
      GAGE,                            // rain gage 雨量计
      SUBCATCH,                        // subcatchment 子流域
      NODE,                            // conveyance system node 运输系统节点
      LINK,                            // conveyance system link 运输系统链接
      POLLUT,                          // pollutant 污染物
      LANDUSE,                         // land use category 土地资源分类
      TIMEPATTERN,                     // dry weather flow time pattern 干燥天气流时间模式
      CURVE,                           // generic table of values 一般值表
      TSERIES,                         // generic time series of values 值的一般时间序列
      CONTROL,                         // conveyance system control rules 运输系统控制规则
      TRANSECT,                        // irregular channel cross-section 不规则的通道截面
      AQUIFER,                         // groundwater aquifer 地下水含水层
      UNITHYD,                         // RDII unit hydrograph RDII单位自记水位计
      SNOWMELT,                        // snowmelt parameter set 融雪参数集
      SHAPE,                           // custom conduit shape 定制的管道形状
      LID,                             // LID treatment units 控制处理单元
      MAX_OBJ_TYPES};
